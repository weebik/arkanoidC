...
int frameCount, timerFPS, lastFrame, lastTime, is_running, custom_map;
...
int main(int argc, char* argv[])
{
    ...
    /*if(argc>0 && argc<=1)
    {
        ...
        if(file==NULL) printf("Failed at opening file")
        else
        {
            if(checkfile) custom_map = 1;
        }
    }
    if(custom_map) customMapInit(file);
    else defaultMapInit();
    fclose(file);*/
    resetGame();    while(is_running!=0)
    {
